...
/*   Updated: 2024/03/28 10:16:04 by jberay           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_cd(t_exec *exec, char **cmd)
{
	...
	if (stat(cmd[1], &buf) == 0)
	{
		...
		{
			if (chdir(cmd[1]) == -1)
				return (error_exit(exec, EXECVE_FAIL, cmd[1]));
		}
		else
			return (error_exit(exec, NO_PATH, cmd[1]));
	}
	else
		return (error_exit(exec, STAT_FAIL, cmd[1]));
	old[0] = "export";
